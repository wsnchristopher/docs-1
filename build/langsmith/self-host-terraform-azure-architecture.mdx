...
LangSmith on Azure deploys in up to five passes. Each pass adds a capability layer on top of the previous. All layers share the same AKS cluster and `langsmith` namespace.

<img src="/images/self-hosted-terraform/azure-architecture.png" alt="LangSmith on Azure deployment passes and service layout" className="block dark:hidden" />
<img src="/images/self-hosted-terraform/azure-architecture-light.png" alt="LangSmith on Azure deployment passes and service layout" className="hidden dark:block" />

| Pass | Layer | What it adds |
|---|---|---|
| 1 | Azure infrastructure | VNet, AKS, Postgres, Redis, Blob, Key Vault, cert-manager, KEDA, ingress controller |
| 2 | LangSmith base | frontend, backend, platform-backend, queue, ingest-queue, ace-backend, clickhouse, playground |
...
| 5 | Insights + Polly | Clio analytics (ClickHouse-backed), Polly eval agent (operator-managed, dynamic) |

## Pass 2 deployment paths
