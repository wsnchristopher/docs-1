...
| Variable | Default | Required | Description |
|---|---|---|---|
| `tls_certificate_source` | `acm` | no | TLS source for in-cluster gateways: `acm`, `letsencrypt` (cert-manager DNS-01 ACME), or `none`. |
...
| `istio_nlb_scheme` | `internet-facing` | no | Scheme for the Istio ingress gateway NLB: `internet-facing` or `internal`. |
...
Sourcing `infra/scripts/setup-env.sh` writes these to AWS SSM Parameter Store. External Secrets Operator syncs them into the cluster as Kubernetes secrets. Never set these inline in `terraform.tfvars`.
