...
`interrupt_config` is optional and must be an object when present. For field semantics and `interrupt_config` usage, see [Reference tools](/langsmith/managed-deep-agents-mcp#reference-tools).
