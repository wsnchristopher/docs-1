...
**Why `max_pods = 30` triggers it:** AKS default is 30 pods per node. Pass 2 alone deploys ~37 pods. The autoscaler tries to add a second node, hits quota, enters backoff. Fix: `default_node_pool_max_pods = 60` in `terraform.tfvars` so all pods fit on one node.
...
**Alternative — switch VM family if DSv3 quota is exhausted:** Use `Standard_DS4_v2` (default) + `Standard_DS5_v2` (large). Same vCPU, slightly less RAM. Validated for full Pass 2 to 5 deploy.
...
### Pass 5 — `backend-ch-migrations` in `CreateContainerConfigError`
