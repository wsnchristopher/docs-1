...
- Upgraded pyjwt from version 2.12.1 to 2.13.0, incorporating multiple security fixes and hardening measures.
- Updated dependencies in the /license_jwt project, including security fixes and improvements by bumping `pyjwt` to 2.13.0, `ruff` to 0.15.14, and `pytest-asyncio` to 1.4.0.
- Updated `hono` from version 4.12.21 to 4.12.25 with critical security fixes.
- Updated `hono` from version 4.12.21 to 4.12.25 to improve security and fix critical middleware vulnerabilities.
- Updated `undici` dependency from version 7.24.7 to 7.28.0 to address multiple security vulnerabilities.
- Resolved `no_such_interrupt` errors in API responses to HITL interrupts by improving interrupt ID validation.
- Re-applied missing indexes by renumbering migration 058 to 061 and added a migration HEAD sentinel to prevent future ordering issues.
- Node.js in Wolfi/Chainguard FIPS Agent Server images now uses FIPS-compliant OpenSSL, ensuring Node crypto operates in FIPS mode.
- Migrated metrics to the OpenTelemetry/Prometheus client, updated metric names for scrapers, and replaced certain gauges with a latency histogram.
- The Agent Server Go core now builds with FIPS 140-3 compliance, using the validated Go Cryptographic Module (GOFIPS140=v1.0.0) by default.
</Update>
