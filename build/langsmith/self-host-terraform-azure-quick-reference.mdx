...
For the full deployment walkthrough, see the [Azure quickstart](/langsmith/self-host-terraform-azure-quickstart) and per-pass guides ([Pass 1](/langsmith/self-host-terraform-azure-pass-1-infrastructure) through [Pass 5](/langsmith/self-host-terraform-azure-pass-5-insights)).

## Deployment overview

| Pass | What gets deployed | Command |
|---|---|---|
| 1 | AKS + Postgres + Redis + Blob + Key Vault + cert-manager + KEDA + ingress | `make apply` |
| 1.5 | Cluster credentials + Kubernetes Secrets from Key Vault | `make kubeconfig && make k8s-secrets` |
| 2 (Helm) | LangSmith Helm (~17 pods) with shell scripts | `make init-values && make deploy` |
| 2 (TF) | LangSmith Helm with Terraform — secrets + SA + Helm release in state | `make init-app && make apply-app` |
| 3 | + LangSmith Deployment — bump `default_node_pool_min_count` to 5 first | `make apply && make init-values && make deploy` |
| 4 | + Agent Builder | `make init-values && make deploy` |
| 5 | + Insights + Polly | `make init-values && make deploy` |
...
enable_deployments   = true           # Pass 3 — listener + operator + host-backend
enable_agent_builder = true           # Pass 4 (requires enable_deployments)
enable_insights      = true           # Pass 5 — Clio analytics
enable_polly         = true           # Pass 5 — Polly AI eval (requires enable_deployments)
```

<Warning>
Pass 3 requires `default_node_pool_min_count = 5` first. Operator-spawned pods need node headroom; without it, agent pods stay in `Pending` indefinitely.
</Warning>
...
For the full teardown procedure, see the [Azure teardown guide](/langsmith/self-host-terraform-azure-teardown).
