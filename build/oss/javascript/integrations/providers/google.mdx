...
The `@langchain/google-vertexai` package provides [`ChatVertexAI`](/oss/javascript/integrations/chat/google_vertex_ai), [`VertexAIEmbeddings`](/oss/javascript/integrations/embeddings/google_vertex_ai), and [`VertexAI`](/oss/javascript/integrations/llms/google_vertex_ai) for Vertex AI on Node.js. It depends on [`@langchain/google-gauth`](#langchaingoogle-gauth) for authentication. This package is superseded by the Vertex AI support built into `@langchain/google` for chat.
