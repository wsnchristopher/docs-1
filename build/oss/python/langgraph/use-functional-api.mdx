...
`@task` also supports `timeout=` for capping how long an attempt may run. See [Fault tolerance](/oss/python/langgraph/timeout-and-error-handling#functional-api) for `TimeoutPolicy` usage with `@task` and `@entrypoint`.
