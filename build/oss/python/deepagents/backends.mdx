...
Python also exposes a `delete` tool for removing a file or, recursively, a directory.
...
| `delete` | `(file_path: str) -> DeleteResult` | Optional. Remove a file or, recursively, a directory. Backends that do not override it inherit a default that raises `NotImplementedError`, and the `delete` tool surfaces that as a "deletion not available" error instead of failing the run. Use `supports_delete()` to check support without triggering it. |
