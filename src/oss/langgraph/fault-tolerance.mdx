...
**The transition is atomic at the superstep boundary**. This is the part that’s painful to get right outside a workflow engine. When the original node fails, its ERROR write is committed to the checkpoint, and the handler task is scheduled as a new task in the same step. 

**Handler crashes resume safely too**. If the handler itself raises—or the host process crashes mid-handler—and a checkpointer is configured, resuming the run re-schedules the handler, not the original failing node. Once a run enters the recovery phase, it stays there.
...
Use @[`set_node_defaults`] on @[`StateGraph`] to configure retry, timeout, cache, and error-handler policies once for every node in the graph. Per-node values passed to @[`add_node`] always override the defaults, and the defaults are applied at `compile()` time:
...
graph = (
    StateGraph(State)
    .set_node_defaults(  # [!code highlight]
        retry_policy=RetryPolicy(max_attempts=3),
...
`set_node_defaults` is a fluent builder method that returns the same @[`StateGraph`], so it chains naturally with `add_node`, `add_edge`, and `compile`. Defaults are not inherited by subgraphs.

### Supported defaults

| `set_node_defaults` kwarg | Fallback for `add_node` kwarg | Applies to error-handler nodes? |
| --- | --- | --- |
| `retry_policy` | `retry_policy` | Yes—a stuck handler is retried like any other node. |
| `timeout` | `timeout` | Yes—a stuck handler is cancelled like any other node. |
...
If the default `error_handler` itself raises, the run fails. The default handler is never invoked on error-handler nodes, so handler failures cannot recurse.
