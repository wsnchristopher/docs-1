...
:::python
Python also provides a `delete` tool, which removes a file or, recursively, a directory and its contents.

<Note>The `delete` tool requires `deepagents>=0.7.0`. Backends that do not implement deletion return a "deletion not available" error instead of the tool succeeding.</Note>
:::
