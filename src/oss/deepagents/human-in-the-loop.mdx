...
- **Configuration object (`{"allowed_decisions": [...]}` / `InterruptOnConfig(...)`)**: Fine-grained control over decisions, prompt description, and conditional interrupt logic. In Python, use a dict or `InterruptOnConfig(...)`. In TypeScript, use an object such as `{ allowedDecisions: [...] }`.
...
## `InterruptOnConfig` and configuration objects

Configuration objects give you precise control over how interrupts behave for a specific tool. In Python, use either a dict such as `{"allowed_decisions": [...]}` or `InterruptOnConfig(...)`. Use `InterruptOnConfig` when configuring Python-only options such as `when`.
...
### Customize descriptions (Python only)

The `description` field customizes the text shown in the interrupt request. Pass a string for static text, or pass a callable that receives the tool call, current agent state, and runtime.

### Add a conditional predicate with `when` (Python only)

The `when` parameter accepts a predicate that controls whether an interrupt fires for a given tool invocation. When `when` returns `False`, the tool call is auto-approved and execution continues without prompting a human. When it returns `True`, or when `when` is omitted, the normal interrupt behavior applies.

Use `when` to interrupt only for high-risk invocations of a tool, rather than every call.
...
| `tool` | In batch mode, `None`; in per-call mode, the `BaseTool` instance |
| `state` | Current agent state |
| `runtime` | In batch mode, the node-level `Runtime`; in per-call mode, a `ToolRuntime` |

The `when` field is optional and backward compatible. Configs without it behave identically to previous versions.
