...
| [Context Hub](#contexthubbackend-langsmith-hub) (Python only) | `agent = create_deep_agent(model="google_genai:gemini-3.1-pro-preview", backend=ContextHubBackend(identifier="my-org/my-agent"))` <br></br>Persist files in a LangSmith Hub agent repo. Files are stored as commits in Context Hub and shared across threads, deployments, and machines. Great for centrally managed agent context (prompts, skills, memories) that you want versioned in LangSmith. |
...
graph TB
    ...
    class Tools trigger
    class Backend,State,Disk,Store,Sandbox,LocalShell,Composite,ContextHub,Custom process
...
:::python
@[`ContextHubBackend`] persists files in a [LangSmith](/langsmith/home) Hub agent repo. Files are written as commits to Context Hub and shared across threads, deployments, and machines.

<BackendContextHubPy />

**How it works:**
- Lazily loads the agent repo's file tree on first access. A missing repo (`pull_agent` returns 404) is treated as empty and created on first write.
- Maintains an in-memory cache of file contents and a commit hash, so subsequent reads are served locally and writes are chained as optimistic commits via `parent_commit`.
- Each `write` or `edit` is committed as a single Hub commit; multi-file uploads are batched into one commit.
- On a mutating failure, the cache is invalidated so the next read re-pulls fresh state from Hub.
- Tracks linked entries (skills, sub-agents, other Hub repos) separately from regular files. Use `get_linked_entries()` to inspect them.

**Best for:**
- Agents whose context (prompts, skills, memories, instructions) is centrally managed in LangSmith.
- Sharing the same versioned file tree across many deployments, threads, and developers.
- Auditing or rolling back agent state via Hub commit history.

<Note>
Requires a LangSmith account and a configured `LANGSMITH_API_KEY`. The `identifier` is the Hub agent repo in `"owner/name"` form (or `"-/name"` for the authenticated user's namespace).
</Note>
:::

:::js
`ContextHubBackend` is currently Python-only. Track [`deepagents`](https://github.com/langchain-ai/deepagents) releases for JS support.
:::
