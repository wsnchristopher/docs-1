...
<CodeGroup>
```bash pip wrap
pip install deepagents langchain langchain-openai langchain-text-splitters requests numpy
```
...
The loaded documentation is long, which makes it too large to fit into the context window of many models.
Even for those models that could fit the full corpus in their context window, models can struggle to find information in very long inputs.
