...
<Note>
The prompt must be a structured prompt (type `StructuredPrompt`), created in the [LangSmith UI](https://smith.langchain.com). A `StructuredPrompt` combines a prompt template with an output schema, ensuring the model returns data in a defined structure.
</Note>
