...
2. **Without Terraform state** — manual cleanup via AWS CLI when state is lost.

Both follow the same reverse-dependency order. Pick the section that matches your situation.

<Warning>
Teardown permanently deletes the RDS instance, S3 bucket contents, and SSM parameters. Export any data you need to retain before proceeding.
</Warning>
