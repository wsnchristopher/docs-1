...
The diagram below shows the basic flow for OpenTelemetry tracing with LangSmith, including the fanout pattern where a single stream of spans is routed to multiple observability backends.
...
The OpenTelemetry SDK instruments your application code and emits spans. Spans travel via the OTLP protocol to an OpenTelemetry Collector, which batches and routes them to one or more destinations simultaneously — this is called **fanout**. LangSmith receives spans at its OTLP endpoint and displays them as traces in the dashboard.
