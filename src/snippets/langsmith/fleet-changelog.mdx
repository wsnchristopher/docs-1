<Update label="June 20-28, 2026" rss={{ title: "2026-06-20 - Fleet product update" }}>

## New features

- Fleet chat now lets you create an access profile directly from the Access Profiles dialog, so you can open the sandboxes create flow when a workspace has none configured.
- Fleet now completes OAuth for MCP servers whose authorization server requires client-secret authentication at the token endpoint, so the consent flow finishes cleanly.
- Fleet now sends the MCP protocol version a server negotiates during the handshake, both when loading tools and when the agent calls them, so servers that require a newer version return tools and accept calls.
