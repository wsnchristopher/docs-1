...
- Fleet now sends the [MCP protocol](/langsmith/fleet/mcp-framework) version a server negotiates during the handshake, both when loading tools and when the agent calls them, so servers that require a newer version return tools and accept calls.
