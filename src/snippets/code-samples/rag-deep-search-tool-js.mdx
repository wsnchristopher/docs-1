...
function makeSearchDocumentationTool(vectorStore: {
  similaritySearch: (query: string, k: number) => Promise<
    Array<{ pageContent: string; metadata: Record<string, string> }>
  >;
}) {
  return tool(
...
const searchDocumentation = makeSearchDocumentationTool(vectorStore);
```
